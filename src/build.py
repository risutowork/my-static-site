...
def build_indexes_from_works(works: List[Dict[str, Any]]):
    ...
    actresses_keys = sorted(actresses.keys())
    genres_keys = sorted(genres.keys())
...
def main():
    ...
    for w in works:
        ...
        if not wid:
            continue
        html = tpl_page.render(site_name=site_name, w=w)
        write_text(OUT / "works" / str(wid) / "index.html", html)

    # トップ
    write_text(OUT / "index.html", tpl_index.render(site_name=site_name, works=works))
    ...
    write_text(
        ...
        tpl_list.render(
            ...
            items=[{"name": a, "href": f"/actresses/{slugify_simple(a)}/"} for a in actresses_keys],
        ),
    )

    # 女優別作品一覧（見た目はindexと同じ）
    for a in actresses_keys:
        write_text(
            OUT / "actresses" / slugify_simple(a) / "index.html",
            tpl_index.render(site_name=site_name, works=actresses[a]),
        )
    ...
    write_text(
        ...
        tpl_list.render(
            ...
            items=[{"name": g, "href": f"/genres/{slugify_simple(g)}/"} for g in genres_keys],
        ),
    )
    ...
    for g in genres_keys:
        write_text(
            ...
            tpl_index.render(site_name=site_name, works=genres[g]),
        )
